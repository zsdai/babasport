...
function login(){
	//另一个系统cookie的路径不一样 cookie不能携带 	127.0.0.1和localhost就不一样了
	
	window.location.href="http://localhost:8082/login.aspx?ReturnUrl="+encodeURIComponent(window.location.href);
}
//由于使用了静态化技术  所以判断用户是否登陆时 不能使用jstl标签来判断 只能使用纯js
$(function(){
	//页面加载时 使用ajax判断用户是否登陆
	$.ajax({
		url:"http://localhost:8082/isLogin.aspx",
		type:"POST",
		dataType: "jsonp",
		/*使用jsonp jquery内部模拟跨域请求,
		如果不适用，浏览器将默认阻止接受返回结果
		*/
	...
	<div class="w">
		...
		<ul class="fr lh">
			<li class="fore1" id="loginbar" clstag="homepage|keycount|home2013|01b">您好！欢迎来到淘淘！
				...
				<a id="register" href="javascript:void(0);">[免费注册]</a>&nbsp;
				<a id="logout" href="javascript:void(0);">[注销]</a>&nbsp;
			</li>
			<li class="fore2 ld" clstag="homepage|keycount|home2013|01c">
				<s></s>
				<a id="myorder" href="http://jd2008.jd.com/JdHome/OrderList.aspx" rel="nofollow">我的订单</a>
			</li>
